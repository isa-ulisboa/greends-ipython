...
# python ASSIGNMENTS\GroupAssignment\Group_3_output\group3_endpoint_Vasco.py --latitude 37.64 --longitude -7.66 --soiltype 3 --pFCritical 3.1 --next24rain_treshold 2 --vpd_treshold 0.5 
...
#ajusting possition of the graphs
fig.subplots_adjust(top=0.84, bottom=0.16, hspace=0.4, wspace=0.3)

#Temperature and VPD graph 
...
#new code to put legend below the chart
axs[0,0].legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), frameon=False)



#EvapoTranspiration Rain Rel Hum graph 
...
#new code to put the 2 legends below the chart
axs[0,1].legend(loc='upper center', bbox_to_anchor=(0.6, -0.05), frameon=False)
secax_0_1.legend(loc='upper center', bbox_to_anchor=(0.4, -0.05), frameon=False)



#Soil Moisture - 3 to 9cm graph 
...
secax_1_0.plot(dates, pF_3_9, color='brown',label='pF')
secax_1_0.bar(dates, [x * inpFCritical for x in plan_3_9], color='red',label='Next irrigation', linewidth=3)
secax_1_0.set_ylabel('pF log(-h)', color='brown')
secax_1_0.tick_params(axis='y', colors='brown')
secax_1_0.plot(dates, [inpFCritical] * len(dates), color='brown',label='Critical pF', linestyle='dashed')
secax_1_0.set_ylim([min(pF_3_9)*0.97, max(pF_3_9)*1.03]) # just for scale purposes
axs[1,0].set_ylabel('% vol', color='black')
axs[1,0].tick_params(axis='x', labelrotation=50)
axs[1,0].xaxis.set_major_locator(MultipleLocator(12))
axs[1,0].set_xlabel('Day Hour')

#new code to put the 2 legends below the chart
axs[1,0].legend(loc='lower center',  bbox_to_anchor=(0.7, -0.57), frameon=False)
secax_1_0.legend(loc='lower center', bbox_to_anchor=(0.3, -0.57) , frameon=False)



#Soil Moisture - 9 to 27cm graph 
...
secax_1_1.plot(dates, pF_9_27, color='brown',label='pF')
secax_1_1.set_ylabel('pF log(-h)', color='brown')
secax_1_1.plot(dates, [inpFCritical] * len(dates), color='brown',label='Critical pF', linestyle='dashed')
...
axs[1,1].tick_params(axis='x', labelrotation=50)
axs[1,1].xaxis.set_major_locator(MultipleLocator(12))
axs[1,1].set_xlabel('Day Hour')

#new code to put the 2 legends below the chart
axs[1,1].legend(loc='lower center',bbox_to_anchor=(0.7, -0.57), frameon=False)
secax_1_1.legend(loc='lower center',bbox_to_anchor=(0.3, -0.57), frameon=False)
