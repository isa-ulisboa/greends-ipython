...
with open("FORGE/JOHANNA/Climate_hisafe_knmievaluation_daily_from_01-01-1981_to_31-12-2010_vpd.csv", "w", newline='') as newfile:
    for i in range(nLines):
        csv.writer(newfile, doublequote=False, escapechar=None, quotechar=None).writerow(climate_info[i])
...
with open("FORGE/JOHANNA/Cimate_hisafe_knmircp85_daily_from_01-01-2006_to_31-12-2100_vpd.csv", "w", newline='') as newfile:
    for i in range(nLines):
        csv.writer(newfile, doublequote=False, escapechar=None, quotechar=None).writerow(climate_info[i])
